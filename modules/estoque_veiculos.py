...
def extrair_placa(texto_completo):
    """Extrai a placa de veículo usando regex aprimorada."""
    if not texto_completo:
        return None

    # Padrão Mercosul
    padrao = r'PLACA[:;\s]*([A-Z]{3}[0-9][A-Z0-9][0-9]{2})'
    match = re.search(padrao, texto_completo, re.IGNORECASE)
    if match:
        return match.group(1).strip()

    # Padrão antigo
    padrao_antigo = r'PLACA[:;\s]*([A-Z]{3}-?[0-9]{4})'
    match = re.search(padrao_antigo, texto_completo, re.IGNORECASE)
    if match:
        return match.group(1).strip()
...
def validar_placa(placa):
    ...
    # Valida formato Mercosul
    if re.fullmatch(r'[A-Z]{3}[0-9][A-Z0-9][0-9]{2}', placa_sem_hifen):
        return True

    # Valida formato antigo
    if re.fullmatch(r'[A-Z]{3}[0-9]{4}', placa_sem_hifen):
        return True
